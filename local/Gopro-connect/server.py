...
def next_session_code(ch: int) -> str:
    ...
    เลขนำหน้า = config.LANE_ID (ตั้งใน .env) ไม่ใช่ ch — กัน code ชนกันตอนมีหลายเครื่อง
    อัปคลิปขึ้นคลาวด์เดียวกัน (แต่ละเครื่องนับ ch1 เป็นเลนของตัวเองเหมือนกันหมด ถ้าใช้ ch
...
class LaneManager:
    ...
    def start_process(self, ch: int) -> WebSession:
        channel = self.channels.get(ch)
        if channel is None:
            raise HTTPException(404, f"ไม่พบช่อง {ch}")
...
def start_process(ch: int):
    session = manager.start_process(ch)
