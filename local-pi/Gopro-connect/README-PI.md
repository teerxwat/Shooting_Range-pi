...
| `server.py` (LocalCleaner) | ลบคลิปใน Pi (`DELETE_LOCAL_AFTER_UPLOAD=1`): ต้นฉบับ 4K ลบทันทีหลังแปลงเสร็จ, ไฟล์เล่นรีเพลย์ลบเมื่ออัปคลาวด์สำเร็จและเซสชันจบ, ลบโฟลเดอร์ดาวน์โหลดว่างทิ้ง — อัปไม่สำเร็จไม่ลบ |

### เน็ตหลุด

ระบบที่เลนใช้งานได้ปกติ (อัด / ดูรีเพลย์ / จบการใช้งาน) แล้วเก็บของที่ต้องส่งไว้ในเครื่องก่อน:
คิวอัปคลิป (`.session/upload_queue.json`), รายงานจบเซสชัน (`.session/session_reports.jsonl`)
เน็ตกลับมา → ส่งย้อนหลังเองภายใน ~1 นาที · ไม่มีการลบไฟล์ในกล้อง/ใน Pi จนกว่าจะอัปสำเร็จ
(ยกเว้นต้นฉบับ 4K ใน Pi ที่ลบหลังแปลงเสร็จ เพราะกล้องยังเก็บต้นฉบับไว้) · ตั้ง PIN ไม่ต้องรอเน็ต
